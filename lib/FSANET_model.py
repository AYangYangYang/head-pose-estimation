...
class BaseFSANet(object):
    ...
    def ssr_S_model_build(self, num_primcaps, m_dim):
        ...
        # 把3个attention map 连接起来得到一个feat_pre_concat
    ...
    def __call__(self):
        ...
        # 构建两个stream网络,输入[b,64,64,3]得到三个[b,8,8,64]
        ...
        # 前向传播过程
        # [b,64,64,3]转换为三个[b,8,8,64]
        ssr_G_list = ssr_G_model(img_inputs)
        #[b, 8, 8, 64]转换为ssr_primcaps[b, 3 * 7, 64]
        ssr_primcaps = ssr_S_model(ssr_G_list)
        # 根据子类的实现不一样，各自不同，但是输出结果为3个[b,16]
        ssr_Cap_list = ssr_aggregation_model(ssr_primcaps)
        # 3个[b,16]转换为3个[b,3,3]和6个[b,3]
        # ssr_F_Cap_list前三个形状为pred_s1,pred_s2,pred_s3[b,3,3]
        # ssr_F_Cap_list中间三个为delta_s1,delta_s2,delta_s3形状为[b,3]
        # ssr_F_Cap_list后面三个为local_s1,local_s2,local_s3
        # 我们使用这个三个变量带入到多阶段回归方程中得到头部姿势角度
